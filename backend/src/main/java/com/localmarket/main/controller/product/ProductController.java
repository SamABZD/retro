...
@RestController
@RequestMapping({"/api/listings", "/api/products"})
...
public class ProductController {
    ...
    public ResponseEntity<ProductResponse> createProduct(
            ...
            @RequestPart(value = "listingStatus", required = false) String listingStatus,
        ...
        if ((images == null || images.length == 0) && (imageUrl == null || imageUrl.trim().isEmpty())) {
        ...
        Set<Long> categoryIds = parseCategoryIds(categoryIdsString);
        ...
        ProductRequest request = ProductRequest.builder()
                ...
                .imageUrl(imageUrl)
                .images(parseExistingImages(existingImages))
                .condition(parseCondition(condition))
                .listingStatus(parseListingStatus(listingStatus))
                .build();

        return ResponseEntity.ok(productService.createProduct(request, images, userDetails.getId()));
    }
    ...
    public ResponseEntity<ProductResponse> updateProduct(
        ...
        return ResponseEntity.ok(productService.updateProduct(id, request, images, userDetails.getId()));
    }
    ...
    public ResponseEntity<Page<ProducerProductsResponse>> getAllProducts(
        ...
        return ResponseEntity.ok(productService.getAllProductsGroupedByProducer(
            pageable, search, parseCondition(condition), minPrice, maxPrice));
    }
    ...
    public ResponseEntity<Page<ProducerProductsResponse>> getProductsByCategory(
        ...
        return ResponseEntity.ok(productService.getProductsByCategory(
            categoryId, pageable, search, parseCondition(condition), minPrice, maxPrice));
    }
    ...
    public ResponseEntity<Resource> getImage(@PathVariable String filename) throws IOException {
        ...
        return ResponseEntity.ok()
            .contentType(org.springframework.http.MediaTypeFactory.getMediaType(filename)
    ...
    @Operation(summary = "Get one of my listings", description = "Get a listing owned by the authenticated seller")
    @SecurityRequirement(name = "cookie")
    @GetMapping("/my-listings/{id}")
    @ProducerOnly
    public ResponseEntity<MyProductResponse> getMyListing(
            @PathVariable Long id,
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        return ResponseEntity.ok(productService.getProducerProduct(id, userDetails.getId()));
    }

    private List<String> parseExistingImages(String value) {
        if (value == null) {
            return null;
        }
        try {
            List<String> urls = objectMapper.readValue(value, new TypeReference<List<String>>() {});
            if (urls == null || urls.size() > 8 || urls.stream().anyMatch(url -> url == null || url.isBlank())) {
                throw new ApiException(ErrorType.INVALID_REQUEST, "Choose between 1 and 8 valid listing images");
            }
            return urls;
        } catch (JsonProcessingException invalidJson) {
            throw new ApiException(ErrorType.INVALID_REQUEST, "Listing images must be a valid list");
        }
    }
    ...
    private BigDecimal parsePrice(String value) {
        try {
            ...
            throw new ApiException(ErrorType.VALIDATION_FAILED, "Listing price must be a number");
        }
    }
    ...
    private Integer parseQuantity(String value) {
        try {
            ...
            throw new ApiException(ErrorType.VALIDATION_FAILED, "Listing quantity must be a whole number");
        }
    }

    private Set<Long> parseCategoryIds(String value) {
        try {
            Set<Long> ids = Arrays.stream(value.split(","))
                .map(String::trim)
                .filter(part -> !part.isEmpty())
                .map(Long::parseLong)
                .collect(Collectors.toSet());
            if (ids.isEmpty()) {
                throw new NumberFormatException("empty categories");
            }
            return ids;
        } catch (NumberFormatException exception) {
            throw new ApiException(ErrorType.VALIDATION_FAILED, "Select a valid listing category");
        }
    }
    ...
    private ListingCondition parseCondition(String value) {
        ...
        try {
            ...
            throw new ApiException(ErrorType.INVALID_REQUEST, "Unsupported listing condition: " + value);
        }
    }

    private ListingStatus parseListingStatus(String value) {
        if (value == null || value.isBlank()) {
            return null;
        }
        try {
            return ListingStatus.valueOf(value.trim().toUpperCase().replace('-', '_').replace(' ', '_'));
        } catch (IllegalArgumentException exception) {
            throw new ApiException(ErrorType.INVALID_REQUEST, "Unsupported listing status: " + value);
        }
    }
} 
