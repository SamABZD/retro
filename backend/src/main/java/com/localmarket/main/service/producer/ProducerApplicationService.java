...
public class ProducerApplicationService {
    ...
    public ProducerApplicationResponse submitApplication(ProducerApplicationRequest request, Long customerId) {
        ...
        if (customer.getRole() != Role.CUSTOMER) {
            throw new ApiException(ErrorType.INVALID_REQUEST, "Only buyers can apply to become sellers");
        }
        ...
        if (request.getCategoryIds() == null || request.getCategoryIds().isEmpty()) {
            throw new ApiException(ErrorType.VALIDATION_FAILED, "Choose at least one selling category");
        }
    ...
    public ProducerApplicationResponse processApplication(Long applicationId, boolean approved, String declineReason, Boolean approveCustomCategory) {
        ...
        if (approved) {
            ...
            try {
                ...
                log.error("Failed to send seller approval email to {}: {}", application.getCustomer().getEmail(), e.getMessage());
            }
            ...
            int newVersion = customer.getTokenVersion() == null
                ? 1 : customer.getTokenVersion() + 1;
    ...
    public ProducerApplicationResponse getCustomerApplication(Long customerId) {
        User customer = userRepository.findById(customerId)
            .orElseThrow(() -> new ApiException(ErrorType.RESOURCE_NOT_FOUND, "Customer not found"));
        ProducerApplication application = applicationRepository.findTopByCustomerOrderByCreatedAtDesc(customer)
    ...
    public ApplicationStatusResponse checkApplicationStatus(Long customerId) {
        ...
        Optional<ProducerApplication> application = applicationRepository.findTopByCustomerOrderByCreatedAtDesc(customer);
