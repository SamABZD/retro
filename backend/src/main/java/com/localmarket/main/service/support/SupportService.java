...
public class SupportService {
    ...
    public void addMessage(Long ticketId, TicketMessageRequest request, User sender) {
        Ticket ticket = ticketRepository.findById(ticketId)
            .orElseThrow(() -> new ApiException(ErrorType.RESOURCE_NOT_FOUND, "Ticket not found"));

        if (sender.getRole() == Role.PRODUCER
                && !ticket.getCreatedBy().getUserId().equals(sender.getUserId())) {
            throw new ApiException(ErrorType.ACCESS_DENIED, "You cannot access this ticket");
        }
        if (ticket.getStatus() == TicketStatus.CLOSED) {
            throw new ApiException(ErrorType.OPERATION_NOT_ALLOWED, "Closed tickets cannot receive messages");
        }
